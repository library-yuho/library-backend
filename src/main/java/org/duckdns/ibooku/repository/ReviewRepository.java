...
public interface ReviewRepository extends JpaRepository<Review, Integer> {
    @Query(value = "SELECT * FROM review WHERE isbn = :isbn AND is_spoiler = :isSpoiler ORDER BY id DESC", nativeQuery = true)
    List<Review> findReviewsByIsbnAndIsSpoilerOrderByIdDesc(@Param("isbn") String isbn, @Param("isSpoiler") boolean isSpoiler);

    @Query(value = "SELECT * FROM review WHERE isbn = :isbn AND is_spoiler = :isSpoiler ORDER BY point DESC", nativeQuery = true)
    List<Review> findReviewsByIsbnAndIsSpoilerOrderByPointDesc(@Param("isbn") String isbn, @Param("isSpoiler") boolean isSpoiler);

    @Query(value = "SELECT * FROM review WHERE isbn = :isbn AND is_spoiler = :isSpoiler ORDER BY point ASC", nativeQuery = true)
